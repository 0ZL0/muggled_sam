...
parser = argparse.ArgumentParser(description="Script used to run SAMURAI (SAM2) on a video")
    ...
    for is_paused, frame_idx, frame in vreader:
        ...
        if curr_state == STATES.PAUSED:
            ...
            if have_track_prompts and not have_user_prompts and is_changed_track_idx:
                ...
                samurai = samurai_list[buffer_select_idx]
                if samurai is not None:
                    is_mem_ok, mask_pred, _, _, _ = samurai.step_video_masking(
                        sammodel,
                        encoded_img,
                        selected_memory_dict["prompt_memory_encodings"],
                        selected_memory_dict["prompt_object_pointers"],
                        selected_memory_dict["previous_memory_encodings"],
                        selected_memory_dict["previous_object_pointers"],
                    )
                    paused_obj_score = 1.0 if is_mem_ok else -1.0
            ...
            if is_changed_track_idx:
                ...
                for objidx in objiter:
                    ...
                    samurai = samurai_list[objidx]
                    if samurai is None:
                        continue

                    is_mem_ok, mask_pred, mem_enc, obj_ptr, _ = samurai.step_video_masking(
                        sammodel,
                        encoded_img,
                        memory_list[objidx].prompts_buffer.memory_history,
                        memory_list[objidx].prompts_buffer.pointer_history,
                        memory_list[objidx].prevframe_buffer.memory_history,
                        memory_list[objidx].prevframe_buffer.pointer_history,
                    )
                    obj_score = 1.0 if is_mem_ok else -1.0
                    tracked_mask_idx = 0
                    mask_preds = mask_pred.repeat(1, 4, 1, 1)
