...
parser = argparse.ArgumentParser(
    description="Script used to run Segment-Anything-V2 using the SAMURAI tracking variant on a video"
)
...
def samurai_video_step(
    ...
    """Run a video tracking step using the SAMURAI algorithm."""
    ...
    with torch.inference_mode():
        ...
        best_idx, best_samurai_iou, _ = samurai_obj.get_best_decoder_results(mask_preds, iou_preds)
        best_mask_pred = mask_preds[:, [best_idx], ...]
        best_iou_pred = iou_preds[:, [best_idx], ...]
        best_obj_ptr = obj_ptrs[:, [best_idx], ...]

        is_ok_mem = samurai_obj._check_memory_ok(obj_score, best_iou_pred, best_samurai_iou)
        memory_encoding = sammodel.memory_encoder(lowres_imgenc, best_mask_pred, obj_score)

    return obj_score, best_idx, best_mask_pred, memory_encoding, best_obj_ptr, is_ok_mem
    ...
    for is_paused, frame_idx, frame in vreader:
        ...
        if tran_state == STATES.SWITCH_PAUSE_ON:
            ...
            if sammodel.check_have_prompts(*prompts):
                ...
                init_mask = maskresults_list[buffer_select_idx].preds[:, [maskresults_list[buffer_select_idx].idx], ...]
                samurai_list[buffer_select_idx] = SimpleSamurai(init_mask)
        ...
        if curr_state == STATES.PAUSED:
            ...
            if have_track_prompts and not have_user_prompts and is_changed_track_idx:
                ...
                samurai_obj = samurai_list[buffer_select_idx]
                if samurai_obj is None:
                    ...
                    paused_obj_score_t, _, paused_mask_preds, _, _, _ = samurai_video_step(
                        samurai_obj, sammodel, encoded_img, **selected_memory_dict
                    )
                    paused_obj_score = float(paused_obj_score_t.squeeze().float().cpu().numpy())
            ...
            if store_prompt_btn.read():
                ...
                if paused_mask_preds is not None:
                    init_mask = paused_mask_preds[:, [paused_mask_idx], ...]
                    samurai_list[buffer_select_idx] = SimpleSamurai(init_mask)
                ui_elems.clear_prompts()
            ...
            if is_changed_track_idx:
                ...
                for objidx in objiter:
                    ...
                    mem_dict = memory_list[objidx].to_dict()
                    samurai_obj = samurai_list[objidx]
                    if samurai_obj is None:
                        obj_score, best_mask_idx, mask_preds, mem_enc, obj_ptr = sammodel.step_video_masking(
                            encoded_img, **mem_dict
                        )
                        is_mem_ok = True
                    else:
                        obj_score, best_mask_idx, mask_preds, mem_enc, obj_ptr, is_mem_ok = samurai_video_step(
                            samurai_obj, sammodel, encoded_img, **mem_dict
                        )
